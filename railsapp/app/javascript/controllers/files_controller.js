...
export default class extends Controller {
  ...
  uploadFile(file) {
    ...
    fetch('/posts/upload_file', options)
      .then((response) => response.json())
      .then((data) => {
        // Postコントローラーからのレスポンス(blobデータ)
        this.previewFile(file, data.id); // 画像プレビューアクションにblobデータのidを受け渡す
      })
      .catch((error) => {
        console.error(error);
      });
  ...
  previewFile(file, id) {
    ...
    fileReader.onload = function () {
      ...
      const imgBoxAttr = {
        // imgBoxに設定する属性
        class: 'file-box inline-flex mx-1 mb-5',
        'data-controller': 'files',
        'data-files-target': 'file_box',
      };
      ...
      const deleteBtnAttr = {
        ...
        'data-action': 'click->files#deleteFile',
      };
